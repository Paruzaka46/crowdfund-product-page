...
const Radio = ({rewards, onClose, onSubmitData}) => {
    ...
    return (
        <div className="relative bg-white md:p-14 mobile:p-7 font-commissioner mb-5 lg:mx-[25%] md:mx-[15%] mobile:mx-5 rounded-md">
            ...
            <div>
                <div className={`border border-dark-grey/50 md:pt-8 mobile:pt-5 rounded-lg mb-5 ${modalPick === 3?" !border-dark-cyan border-2":null}`}>
                    <div className="flex items-center mb-7 md:px-8 mobile:px-5 md:pt-0 mobile:pt-3">
                        <input onClick={() => {changePick(3)}} className=" mr-4 border-0 w-5 h-5 accent-dark-cyan" type="radio" id="reward1" name="reward" checked={modalPick === 3?true:null}/>
                        <label className="text-[17px] font-bold hover:text-moderate-cyan cursor-pointer" htmlFor="reward1">Pledge with no reward</label>
                    </div>
                    <p className="md:ml-[70px] mobile:ml-5 text-dark-grey pb-5">
                    Choose to support us without a reward if you simply believe in our project. As a backer, you will be signed up to receive product updates via email.
                    </p>
                    <hr className="relative mt-4"/>
                    <div className={`flex md:flex-row mobile:flex-col justify-center items-center ${modalPick === 3?"md:m-8 mobile:m-4 h-[89.2px]":"m-0 h-0 overflow-hidden"} transition-all`}>
                        <button onClick={() => {
                            onSubmitData(0)
                            setNumber('')
                            }} className="bg-moderate-cyan hover:bg-dark-cyan py-[12px] px-7 text-[16px] flex items-center rounded-full text-white font-semibold transition-colors">Continue</button>
                    </div>
                </div>
                    ...
                    <div className={`flex md:flex-row mobile:flex-col justify-between items-center ${modalPick === index?"md:m-8 mobile:m-6 h-[89.2px]":"m-0 h-0 overflow-hidden"} transition-all`}>
                        ...
                        <form onSubmit={(event) => {
                            ...
                            <div className="flex items-center">
                                <i className={`absolute translate-x-5 ${modalPick === index?" inline":"hidden"} text-dark-grey`}><FaDollarSign/></i>
                                <input min={reward.pledge} onChange={(e) => {
                                    e.preventDefault();
                                    handleChange(e)
                                }} className=" pl-10 outline-dark-grey/50 border-2 rounded-full py-[11px] px-7 w-28 mr-4 focus:outline-dark-cyan [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" type="number" value={number}/>
                            </div>
