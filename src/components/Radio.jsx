...
const Radio = ({rewards, onClose}) => {
    const modalPick = usePick()
    const changePick = usePickUpdate()
    const desktop = useMediaQuery({query: "(min-width: 768px)"})

    return (
        <div className="relative bg-white md:p-14 mobile:p-7 font-commissioner mb-5">
            ...
            <div>
                <div className={`border border-dark-grey/50 md:p-8 mobile:p-5 rounded-lg mb-5`}>
                    <div className="flex items-center mb-7 md:pt-0 mobile:pt-3">
                        <input className=" mr-4 border-0 w-5 h-5 accent-dark-cyan" type="radio" id="reward1" name="reward"/>
                ...
                {rewards.map((reward, index) => {
                    return (
                <div key={index} className={`border border-dark-grey/50 md:pt-8 mobile:pt-5 rounded-lg mb-5 ${reward.stocks?null:"opacity-50"}`}>
                    <div className="flex justify-between md:px-8 mobile:px-5">
                        <div className="flex items-center mb-3">
                            <input onClick={() => {changePick(-1)}} className="mr-4 border-0 w-5 h-5 accent-dark-cyan" type="radio" id={`reward ${index+2}`} name="reward" disabled={reward.stocks?false:true} checked={modalPick === index?true:null}/>
                    ...
                    <div className="flex md:flex-row mobile:flex-col justify-between items-center md:m-8 mobile:m-6 gap-4">
                        <p>Enter your pledge</p>
                        <form className="flex items-center justify-center">
                            <input className=" outline-dark-grey/50 border-2 rounded-full py-[11px] px-7 w-28 mr-4 focus:outline-dark-cyan [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" type="number"/>
